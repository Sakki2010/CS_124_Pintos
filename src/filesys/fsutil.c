...
void fsutil_cat(char **argv) {
    ...
    file = filesys_open_file(file_name, NULL);
    if (file == NULL)
...
void fsutil_rm(char **argv) {
    ...
    if (!filesys_remove(file_name, root))
...
void fsutil_extract(char **argv UNUSED) {
    ...
    for (;;) {
        ...
        if (type == USTAR_EOF) {
            ...
            if (!filesys_create_file(file_name, size, NULL))
                PANIC("%s: create failed", file_name);
            dst = filesys_open_file(file_name, NULL);
            if (dst == NULL)
                PANIC("%s: open failed", file_name);
...
void fsutil_append(char **argv) {
    ...
    /* Open source file. */
    src = filesys_open_file(file_name, NULL);
    if (src == NULL)
        PANIC("%s: open failed", file_name);
