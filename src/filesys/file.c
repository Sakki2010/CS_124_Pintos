...
file_t *file_open(inode_t *inode) {
...
file_t *file_reopen(file_t *file) {
...
inode_t *file_get_inode(file_t *file) {
    return file->inode;
}
...
off_t file_read(file_t *file, void *buffer, off_t size) {
...
/*! Reads SIZE bytes from FILE into BUFFER, starting at offset FILE_OFS in the
    ...
    unaffected. */
off_t file_read_at(file_t *file, void *buffer, off_t size,
                   off_t file_ofs) {
...
off_t file_write(file_t *file, const void *buffer, off_t size) {
...
off_t file_write_at(file_t *file, const void *buffer, off_t size,
                    off_t file_ofs) {
...
void file_deny_write(file_t *file) {
    ASSERT(file != NULL);
    if (!file->deny_write) {
...
void file_allow_write(file_t *file) {
    ASSERT(file != NULL);
    if (file->deny_write) {
        file->deny_write = false;
...
/*! Sets the current position in FILE to NEW_POS bytes from the
    start of the file. */
void file_seek(file_t *file, off_t new_pos) {
    ASSERT(file != NULL);
    ASSERT(new_pos >= 0);
    file->pos = new_pos;
}
