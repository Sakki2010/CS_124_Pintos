...
/*! Finds the first differing characters in strings A and B, comparing at most
    the first N bytes of each.
    Returns a positive value if the character in A (as an unsigned
    char) is greater, a negative value if the character in B (as
    an unsigned char) is greater, or zero if strings A and B are
    equal. */
int strncmp(const char *a_, const char *b_, size_t n) {
    const unsigned char *a = (const unsigned char *) a_;
    const unsigned char *b = (const unsigned char *) b_;
...
/*! Copies string SRC to DST.  If SRC is longer than SIZE - 1
    ...
    https://www.millert.dev/papers/strlcpy/ for
