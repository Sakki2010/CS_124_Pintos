...
static list_t *find_bucket (struct hash *, struct hash_elem *);
static struct hash_elem *find_elem (struct hash *, list_t *,
                                    struct hash_elem *);
static void insert_elem (struct hash *, list_t *, struct hash_elem *);
...
hash_clear (struct hash *h, hash_action_func *destructor)
{
  ...
  for (i = 0; i < h->bucket_cnt; i++)
    {
      list_t *bucket = &h->buckets[i];

      if (destructor != NULL)
        while (!list_empty (bucket))
          {
            list_elem_t *list_elem = list_pop_front (bucket);
...
{
  list_t *bucket = find_bucket (h, new);
...
find_elem (struct hash *h, list_t *bucket, struct hash_elem *e)
{
  list_elem_t *i;
...
rehash (struct hash *h)
{
  ...
  list_t *new_buckets, *old_buckets;
  size_t i;
...
static void
insert_elem (struct hash *h, list_t *bucket, struct hash_elem *e)
{
  h->elem_cnt++;
