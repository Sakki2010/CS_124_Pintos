...
struct desc {
    ...
    list_t free_list;      /*!< List of free blocks. */
    lock_t lock;           /*!< Lock. */
};
...
typedef struct block {
    list_elem_t free_elem; /*!< Free list element. */
} block_t;
...
static struct arena *block_to_arena(block_t *);
static block_t *arena_to_block(struct arena *, size_t idx);
...
void * malloc(size_t size) {
    ...
    if (list_empty (&d->free_list)) {
        ...
        for (i = 0; i < d->blocks_per_arena; i++) {
            block_t *b = arena_to_block(a, i);
            list_push_back(&d->free_list, &b->free_elem);
        }
    }

    /* Get a block from free list and return it. */
    b = list_entry(list_pop_front (&d->free_list), block_t, free_elem);
    a = block_to_arena(b);
    a->free_cnt--;
    lock_release(&d->lock);
    return b;
...
static struct arena * block_to_arena(block_t *b) {
...
static block_t * arena_to_block(struct arena *a, size_t idx) {
    ASSERT(a != NULL);
    ASSERT(a->magic == ARENA_MAGIC);
    ASSERT(idx < a->desc->blocks_per_arena);
    return (block_t *) ((uint8_t *) a
                             + sizeof *a
