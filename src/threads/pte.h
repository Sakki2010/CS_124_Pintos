...
#define _AVL2 0x400
#define _AVL3 0x800
#define SUP_TYPE (_AVL1 | _AVL2)
/*! Whether the user should see an entry as present. */
#define SUP_IS_PRESENT(pte) (((pte) & SUP_TYPE) != 0)
/*! Whether the page is backed by a file. */
#define SUP_IS_FILEBACKED(pte) (((pte) & _AVL1) != 0)
/*! Whether you are allowed to write to the backing file. Implies
    SUP_IS_FILEBACKED */
#define SUP_IS_FILEWRITE(pte) (((pte) & SUP_TYPE) == SUP_TYPE)
/*! Sets the type bits (first two AVL bits) to represent no mapping. */
#define SUP_UNMAPPED(pte) ((pte) & ~SUP_TYPE)
/*! Sets the type bits (first two AVL bits) to represent an anonymous mapping. */
#define SUP_ANONYMOUS(pte) (((pte) & ~_AVL1) | _AVL2)
/*! Sets the type bits (first two AVL bits) to represent a read-only file 
    mapping. */
#define SUP_FILEREAD(pte) (((pte) & ~_AVL2) | _AVL1)
/*! Sets the type bits (first two AVL bits) to represent a read-write file 
    mapping. */
#define SUP_FILERW(pte) ((pte) | SUP_TYPE)
/*! Bit to represent if the entry is locked. Used as a backup to ensure locking
    via a busy lock in case memory allocation fails. */
#define SUP_LOCK _AVL3
