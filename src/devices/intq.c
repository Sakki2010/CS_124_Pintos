...
static void wait (struct intq *q, thread_t **waiter);
static void signal (struct intq *q, thread_t **waiter);
...
static void
wait (struct intq *q UNUSED, thread_t **waiter) 
{
  ASSERT (!intr_context ());
  ASSERT (intr_get_level () == INTR_OFF);
...
/* WAITER must be the address of Q's not_empty or not_full
   ...
   the waiting thread. */
static void
signal (struct intq *q UNUSED, thread_t **waiter) 
{
  ASSERT (intr_get_level () == INTR_OFF);
  ASSERT ((waiter == &q->not_empty && !intq_empty (q))
          || (waiter == &q->not_full && !intq_full (q)));
