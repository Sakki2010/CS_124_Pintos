...
struct partition {
    block_t *block;                /*!< Underlying block device. */
    block_sector_t start;               /*!< First sector within device. */
};

static struct block_operations partition_operations;

static void read_partition_table(block_t *, block_sector_t sector,
                                 block_sector_t primary_extended_sector,
                                 int *part_nr);
static void found_partition(block_t *, uint8_t type,
...
void partition_scan(block_t *block) {
    int part_nr = 0;
...
/*! Reads the partition table in the given SECTOR of BLOCK and scans it for
    partitions of interest to Pintos.

    If SECTOR is 0, so that this is the top-level partition table on BLOCK,
    then PRIMARY_EXTENDED_SECTOR is not meaningful; otherwise, it should
    designate the sector of the top-level extended partition table that was
    traversed to arrive at SECTOR, for use in finding logical partitions
    (see the large comment below).

    PART_NR points to the number of non-empty primary or logical partitions
    already encountered on BLOCK.  It is incremented as partitions are found. */
static void read_partition_table(block_t *block, block_sector_t sector,
                                 block_sector_t primary_extended_sector,
                                 int *part_nr) {
    ...
    for (i = 0; i < sizeof pt->partitions / sizeof *pt->partitions; i++) {
        ...
        if (e->size == 0 || e->type == 0) {
            ...
            /* The interpretation of the offset field for extended partitions
               is bizarre.  When the extended partition table entry is in the
               master boot record, that is, the device's primary partition
               table in sector 0, then the offset is an absolute sector number.
               Otherwise, no matter how deep the partition table we're reading
               is nested, the offset is relative to the start of the extended
...
/*! We have found a primary or logical partition of the given TYPE on BLOCK,
    starting at sector START and continuing for SIZE sectors, which we are
    giving the partition number PART_NR.  Check whether this is a partition of
    interest to Pintos, and if so then add it to the proper element of
    partitions[]. */
static void found_partition(block_t *block, uint8_t part_type,
                            block_sector_t start, block_sector_t size,
                            int part_nr) {
    if (start >= block_size(block)) {
        ...
        printf("%s%d: Partition end (%"PRDSNu") past end of "
               "device (%"PRDSNu")\n", block_name(block), part_nr,
               start + size, block_size(block));
    }
    else {
...
/*! Write sector SECTOR to partition P from BUFFER, which must contain
    BLOCK_SECTOR_SIZE bytes.  Returns after the block has acknowledged
    receiving the data. */
