...
/*! Keys that produce the same characters regardless of whether the Shift keys
    are down.  Case of letters is an exception that we handle elsewhere. */
...
static void keyboard_interrupt(struct intr_frame *args UNUSED) {
    ...
    if (code == 0x3a) {
        ...
        if (!release) {
            ...
            /* Handle Alt by setting the high bit.  This 0x80 is unrelated to
               the one used to distinguish key press from key release. */
            if (alt)
                c += 0x80;
...
/*! Scans the array of keymaps K for SCANCODE.
    If found, sets *C to the corresponding character and returns true.
